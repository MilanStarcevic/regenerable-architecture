...
**Risk:** AI implementation decay accumulates invisibly. There is no mechanism to detect it, and no safe path to
regeneration.
...
- Signal dashboard warnings trigger a regeneration review in the team's normal cadence
...
| Level 4 | Fitness functions measure decay signals but thresholds are never acted on — see [Fitness Function Theater](anti-patterns.md) |
...
discipline only to the capsules where AI implementation decay is most likely to accumulate or where the cost of
